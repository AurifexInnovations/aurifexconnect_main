...
public class AttendanceHelper {
    ...
    public AttendanceResponse checkIn(Param param) {
        ...
        Optional<Attendance> existing = attendanceRepository.findByUser_IdAndDate(userId, today);
    ...
    public AttendanceResponse updateAttendance(AttendanceRequest request) {
        ...
        if (request.getId() > 0) {
            ...
            Optional<Attendance> existing = attendanceRepository.findByUser_IdAndDate(request.getUserId(), request.getDate());
            if (existing.isPresent()) {
    ...
    public List<AttendanceResponse> getMonthlyReport(AttendanceRequest request) {
        ...
        if (records.isEmpty()) {
            throw new AttendanceNotFoundException("No records found for " + month);
        }
    ...
    public AttendanceSummaryChartResponse getAttendanceSummaryAnalytics(AttendanceRequest request) {
        ...
        while (!currentDate.isAfter(toDate)) {
            ...
            for (Attendance attendance : attendanceList) {
                if (attendance.getDate().isEqual(currentDate)) {
                    isPresent = true;
                    break;
                }
            }
