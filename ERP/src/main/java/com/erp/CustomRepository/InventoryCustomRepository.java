...
//                    v.id ,
//                    v.stock_quantity ,
//                    v.selling_price_type ,
//                    v.selling_price ,
//                    v.purchase_price_type ,
//                    v.purchase_price ,
//                    v.unit_type ,
//                    v.unit_type_value ,
//                    v.measurement_type ,
...
//    }
    ...
    public ResultDto<InventoryAndBranchProjection> getInventoryDetails(FilterRequest filterRequest) {
        ...
        // ================= GROUP BY =====================
        sql.append(" GROUP BY i.item_id, b.branch_name, t.tax_name, f.file_url, v.id");
        ...
        // ⭐⭐⭐ IMPORTANT: Grouping by itemId ⭐⭐⭐
        Map<Long, InventoryAndBranchProjection> inventoryMap = new HashMap<>();

        for (Object[] r : resultRows) {
            long itemId = ((Number) r[0]).longValue();

            InventoryAndBranchProjection obj = inventoryMap.get(itemId);
            if (obj == null) {
                obj = new InventoryAndBranchProjection();
                ...
                obj.setSkuCode((String) r[9]);
                obj.setEan((String) r[10]);
                obj.setReturnable(r[11] != null && (Boolean) r[11]);
                obj.setTaxId(r[12] != null ? ((Number) r[12]).longValue() : null);
                obj.setProductStatus(r[13] != null ? ProductStatus.valueOf((String) r[13]) : null);
                obj.setBranchId(r[14] != null ? ((Number) r[14]).longValue() : null);
                obj.setCreatedAt(r[15] != null ? ((Timestamp) r[15]).toLocalDateTime() : null);
                ...
                obj.setVarientDtoList(new ArrayList<>());

                inventoryMap.put(itemId, obj);
            }

            // ⭐⭐⭐ Add Variant — Only if exists ⭐⭐⭐
            if (r[22] != null) {
                VarientDto variant = new VarientDto(
                        ...
                        (String) r[26],
                        r[27] != null ? ((Number) r[27]).doubleValue() : 0,
                        (String) r[28],
                        r[29] != null ? ((Number) r[29]).doubleValue() : 0,
                        (String) r[30],
                        r[31] != null ? ((Number) r[31]).doubleValue() : 0,
                        r[32] != null ? ((Timestamp) r[32]).toLocalDateTime() : null
                );

                obj.getVarientDtoList().add(variant);
            }
        }

        // ================== FINAL RESULT =======================
        List<InventoryAndBranchProjection> responseList =
                new ArrayList<>(inventoryMap.values());
    ...
    public ResultDto<InventoryAndBranchProjection> getAllInventory() {
        log.info("Into [InventoryCustomRepository] [getAllInventory]");
        ...
        String sql = """
                ...
                LEFT JOIN tax t ON i.tax_id = t.id
                WHERE i.active = true
                GROUP BY i.item_id, b.branch_name, t.tax_name, f.file_url
        ...
        log.info("Exit [InventoryCustomRepository] [getAllInventory] count={}", totalCount);
        return resultDto;
    }
}
